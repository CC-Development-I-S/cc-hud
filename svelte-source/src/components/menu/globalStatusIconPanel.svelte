...
<Panel name={'Global Status Icons Settings'} icon={faGlobe} color={"white"} bind:group>
  ...
  <div class="mx-4 grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6 items-end">
    ...
    {#if $PlayerHudUIStore.globalIconSettings.dashes != null}
      <div>
        <p class="text-base text-center mb-2">Dashes</p>
        <NumberInput min={0} max={30} bind:value={$PlayerHudUIStore.globalIconSettings.dashes}
          handleUpdateFunction={PlayerHudUIStore.updateAllDashes}
        />
      </div>
    {/if}

    {#if $PlayerHudUIStore.globalIconSettings.gap != null}
      <div>
        <p class="text-base text-center mb-2">Dash Gap</p>
        <NumberInput min={0} max={30} bind:value={$PlayerHudUIStore.globalIconSettings.gap}
          handleUpdateFunction={PlayerHudUIStore.updateAllDashGaps}
        />
      </div>
    {/if}

    {#if $PlayerHudUIStore.globalIconSettings.borderGap != null}
      <div>
        <p class="text-base text-center mb-2">Border Gap</p>
        <NumberInput min={0} max={1} bind:value={$PlayerHudUIStore.globalIconSettings.borderGap} step={0.01}
          handleUpdateFunction={PlayerHudUIStore.updateAllBorderGap}
        />
      </div>
    {/if}
    ...
    {#if $PlayerHudUIStore.globalIconSettings.borderSize != null}
      <div>
        <p class="text-base text-center mb-2">Border Size</p>
        <NumberInput min={0} max={20} bind:value={$PlayerHudUIStore.globalIconSettings.borderSize} step={0.5}
          handleUpdateFunction={PlayerHudUIStore.updateAllBorderSize}
        />
      </div>
    {/if}
  </div>
