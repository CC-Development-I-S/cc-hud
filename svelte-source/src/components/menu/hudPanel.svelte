...
<div class="text-sm flex flex-col text-[#e8e8e8]">
  <div class="relative mx-4 mb-5 mt-3">
    <div class="absolute right-10 top-10 w-[15%] select-none pointer-events-none">
      <img src={img} alt="logo" class="imgur"/>
    </div>
    <div class="-mx-4 mb-4 text-2xl text-white">
      <p>Reset HUD </p>
    </div>
  
    <Button name="Reset Hud" buttonClass="whitespace-nowrap hover:bg-red-600 py-4" disable={$MenuStore.isRestarting}
      disableText={"Resetting Hud..."}
      on:click={() => {fetchNui("restartHud"); $MenuStore.isRestarting = true;}}
    />
    <p class="text-base">Are you having issues with your HUD? Reset it here, or type /resethud</p>

    <Button name="Reset Settings" buttonClass="hover:bg-red-600 mt-4 py-4"
      on:click={() => {
        MenuStore.resetHudMenuSetting();
        // Send updated menu settings to client lua
        MenuStore.sendMenuSettingsToClient();
    }}/>
    <p class="text-base">Wanna revert your settings back to default? Some changes might only change after relogging.</p>
  </div>
